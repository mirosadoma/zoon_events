...
final readonly class CancelEvent
{
    ...
    public function execute(TenantContext $context, Event $event, string $reason): Event
    {
        return DB::transaction(function () use ($context, $event, $reason): Event {
            if (! EventStatus::from($event->status)->canTransitionTo(EventStatus::Cancelled)) {
                throw Phase1Problem::make('event_not_publishable');
            }
            $event->forceFill(['status' => 'cancelled', 'cancelled_at' => now()])->save();
            $this->audit->writeTenant('event.cancelled', 'succeeded', $context, targetType: 'event', targetId: $event->id, metadata: ['reason' => $reason]);
