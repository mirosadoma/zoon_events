...
class AdminPagesAuthTest extends TestCase
{
    ...
    public function test_guest_is_redirected_from_admin_pages(): void
    {
        ...
        $this->get('/admin/tenant-settings')->assertRedirect('/login');
        $this->get('/admin/audit-logs')->assertRedirect('/login');
        $this->get('/tenant/events/evt_1/reports')->assertRedirect('/login');
    }

    public function test_tenant_administrator_can_view_admin_and_report_pages(): void
    {
        ...
        $this->get('/admin/users')
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page->component('admin/Users')->has('users'));

        $this->get('/admin/roles')
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page->component('admin/Roles')->has('roles'));

        $this->get('/admin/tenant-settings')
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page->component('admin/TenantSettings')->has('tenant'));

        $this->get('/admin/audit-logs')
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page->component('admin/AuditLogs')->has('auditLogs'));

        $this->get("/tenant/events/{$event->id}/reports")
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page->component('tenant/reports/EventReport')->has('report'));
    }
    ...
    public function test_member_without_role_is_forbidden(): void
    {
        ...
        $this->get('/admin/tenant-settings')->assertForbidden();
        $this->get('/admin/audit-logs')->assertForbidden();
        $this->get("/tenant/events/{$event->id}/reports")->assertForbidden();
    }
    ...
    private function adminFixture(): array
    {
        ...
        $event = Event::query()->create([
            'tenant_id' => $fixture['tenant']->id,
            'slug' => 'report-event',
            'name_en' => 'Report Event',
            'name_ar' => 'فعالية التقارير',
            'tier' => 'public',
            'status' => 'published',
            'timezone' => 'Africa/Cairo',
            'start_at' => now()->addMonth(),
